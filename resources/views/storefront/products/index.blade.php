        ...
        <div class="flex flex-col lg:flex-row gap-5 lg:gap-6 items-start relative">
                        ...
                        @forelse ($categories as $category)
                            <li>
                                <label class="flex items-center gap-2 cursor-pointer py-1 group">
                                    <input type="checkbox" value="{{ $category->id }}" x-model="selectedCategories" class="w-3.5 h-3.5 rounded border-slate-300 text-[#C1121F] focus:ring-[#C1121F] accent-[#C1121F]">
                                    <span class="group-hover:text-[#C1121F] transition-colors" :class="selectedCategories.includes('{{ $category->id }}') ? 'text-[#C1121F]' : ''">{{ $category->name }}</span>
                                </label>
                            </li>
                        @empty
            ...
            <div class="flex-1 w-full min-w-0">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
                    <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Danh sách sản phẩm <span class="text-sm text-slate-400 font-bold ml-1">({{ $products->count() }})</span></h1>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                    @forelse ($products as $product)
                        ...
                        @endphp
                        <div @click="window.location.href='/products/{{ $product->id }}'" class="bg-white rounded-xl p-2 sm:p-2.5 shadow-sm hover:shadow-md transition-all duration-300 border border-slate-100 group flex flex-col cursor-pointer min-w-0">
                            <div class="relative overflow-hidden rounded-lg bg-slate-50 aspect-[4/5] mb-2 shrink-0">
                                ...
                                <img src="{{ $imageUrl }}" alt="{{ $product->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?q=80&w=400';">
                            ...
                            <div class="px-0.5 flex flex-col flex-grow justify-end min-w-0">
                                <h3 class="font-bold text-slate-800 text-[11px] line-clamp-1 mb-1 group-hover:text-[#C1121F] transition-colors">{{ $product->name }}</h3>
                                <p class="text-[10px] text-slate-400 mb-1 line-clamp-2">{{ $product->category?->name ?? 'Chưa phân loại' }}</p>
                                <div class="flex flex-col leading-none">
                                    <span class="text-[#C1121F] font-black text-[13px]">{{ number_format($product->price, 0, ',', '.') }}đ</span>
                                    <span class="text-slate-400 text-[9px] font-semibold mt-0.5">{{ $product->quantity }} còn hàng</span>
                                </div>
                            </div>
                        </div>
                    @empty
