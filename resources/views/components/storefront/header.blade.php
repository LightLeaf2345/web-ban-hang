<div class="fixed top-2 left-0 right-0 z-50 px-4 flex justify-center w-full">
    <header class="w-full max-w-7xl bg-[#F6F5F2]/90 backdrop-blur-xl shadow-lg shadow-slate-200/50 rounded-full px-4 py-2.5 flex items-center justify-between border border-white/60">
        ...
        <nav class="hidden md:flex items-center gap-8 font-bold text-[15px] ml-8 shrink-0 relative">
            ...
            <a href="/" class="transition-colors {{ request()->is('/') ? 'text-[#C1121F]' : 'text-slate-700 hover:text-[#C1121F]' }}">
                Trang Chủ
            </a>

            <div class="group py-4 -my-4">
                <a href="/products" class="flex items-center gap-1.5 transition-colors cursor-pointer group-hover:text-[#C1121F] {{ request()->is('products*') ? 'text-[#C1121F]' : 'text-slate-700 hover:text-[#C1121F]' }}">
                    Sản Phẩm
                    <svg class="w-4 h-4 transition-transform group-hover:rotate-180 duration-300 {{ request()->is('products*') ? 'text-[#C1121F]' : 'text-slate-500 group-hover:text-[#C1121F]' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"></path></svg>
                </a>
            ...
            <a href="/collections" class="transition-colors {{ request()->is('collections*') || request()->is('flash-sale*') ? 'text-[#C1121F]' : 'text-slate-700 hover:text-[#C1121F]' }}">
                Bộ Sưu Tập
            </a>
