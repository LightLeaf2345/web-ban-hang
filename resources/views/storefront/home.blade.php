        ...
        <section class="relative z-10 pt-0">
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <span class="hidden sm:inline-block bg-[#FFB703] text-black text-xs font-black px-3 py-1.5 rounded-full shadow-sm">{{ $products->count() }} sản phẩm mới</span>
                </div>
            ...
            <div class="flex overflow-x-auto gap-4 md:gap-5 pb-8 hide-scroll snap-x">
                @forelse ($products as $product)
                    ...
                    @endphp
                    <div onclick="window.location.href='/products/{{ $product->id }}'" class="cursor-pointer min-w-[250px] w-[250px] bg-white rounded-[2rem] p-3 shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 group flex flex-col snap-start shrink-0">
                        ...
                        <div class="px-2 pb-2 flex flex-col flex-grow justify-end">
                            <h3 class="font-bold text-slate-800 text-[15px] line-clamp-1 mb-1.5 group-hover:text-[#C1121F] transition-colors">{{ $product->name }}</h3>
                            <p class="text-[12px] text-slate-400 mb-1.5 line-clamp-1">{{ $product->category?->name ?? 'Chưa phân loại' }}</p>
                            <div class="flex items-end gap-2">
                                <span class="text-[#C1121F] font-black text-xl">{{ number_format($product->price, 0, ',', '.') }}đ</span>
                            </div>
                        </div>
                    </div>
                @empty
